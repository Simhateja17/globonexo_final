...
const FAQItem = ({ question, answer, isOpen, onClick }: FAQItemProps) => {
  return (
    <div 
      className="w-full bg-[#141414] rounded-xl p-5 lg:p-6 cursor-pointer transition-all duration-300"
      onClick={onClick}
    >
      <div className="flex items-center justify-between gap-4">
        <span className="text-sm font-normal leading-[22px] text-[#F0F0F0]">
          {question}
        </span>
        <svg
          className={`w-4 h-4 flex-shrink-0 text-[#BFBFBF] transition-transform duration-300 ${
            isOpen ? 'rotate-180' : ''
          }`}
          viewBox="0 0 16 16"
          fill="none"
